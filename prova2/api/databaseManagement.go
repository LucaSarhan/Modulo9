...
var database = func () string {
	...
	dbPath := filepath.Join(parentDir, "database/database.db")

	return dbPath
}

func PostSensor(sensor string, value string) string {
	db, _ := sql.Open("sqlite3", database())
	defer db.Close()

	stmt := `INSERT INTO sensorInfo(sensor, value) VALUES (?, ?)`
...
func GetSensors() []SQLSensor {
	db, _ := sql.Open("sqlite3", database())
	defer db.Close()

	row, err := db.Query("SELECT * FROM sensorInfo ORDER BY sensor")
	if err != nil {
		fmt.Println(err)
	}
	defer row.Close()
